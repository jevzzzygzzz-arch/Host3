# -*- coding: utf-8 -*-
"""
ATX Hosting - XHOST Bot v7.1
Fixed:
- Better detection of AI "thinking" responses (fallback to template)
- Shorter, more direct AI prompt
- All English
- All missing functions added: save_user_file, remove_user_file_db,
  remove_subscription_db, execute_broadcast
"""
import telebot
import subprocess
import os
...
for p in [RAILWAY_VOLUME_PATH, '/data', os.environ.get('RAILWAY_VOLUME_PATH', '')]:
    if p and os.path.isdir(p) and os.access(p, os.W_OK):
        BASE_DIR = p
        print(f"Using volume: {BASE_DIR}")
        break
if not BASE_DIR:
    BASE_DIR = os.path.abspath(os.path.dirname(__file__))
    print(f"Using ephemeral storage: {BASE_DIR}")
...
#  TOKEN CONFLICT
# ==========================================================
PARENT_BOT_TOKEN = TOKEN


def check_token_conflict(file_content_bytes, filename):
    try:
        text = file_content_bytes.decode('utf-8', errors='ignore')
    except:
        return False, ""
    token_pattern = r'["\'](\d{8,12}:[A-Za-z0-9_-]{30,45})["\']'
    matches = re.findall(token_pattern, text)
    for found_token in matches:
        if found_token == PARENT_BOT_TOKEN:
            return True, (
                f"Script `{filename}` contains the SAME BOT_TOKEN as ATX Hosting!\n\n"
                f"This will cause conflict. Use os.environ.get('BOT_TOKEN') instead."
            )
        if found_token.count(':') == 1:
            return True, (
                f"Script `{filename}` has a hardcoded BOT_TOKEN.\n\n"
                f"Use os.environ.get('BOT_TOKEN') instead."
            )
    if re.search(r'telebot\.TeleBot\s*\(\s*["\']', text):
        return True, (
            f"Script `{filename}` hardcodes a token in telebot.TeleBot().\n\n"
            f"Use os.environ.get('BOT_TOKEN')."
        )
    return False, ""
...
AI_SEEK_HEADERS = {
    'User-Agent': "okhttp/4.12.0", 'Accept': "text/event-stream",
...
AI_SEEK_MODEL_LABELS = {
    "qwen/qwen-coder-32b": "Qwen Coder 32B", "openai/gpt-5-mini": "GPT-5 Mini",
    "deepseek/deepseek-chat": "DeepSeek Chat", "grok/grok-4-fast": "Grok 4 Fast",
    "qwen/qwen3-32b": "Qwen 3 32B", "google/gemini-2.5-flash-lite": "Gemini 2.5 Flash Lite",
    "meta-llama/llama-4-scout": "Llama 4 Scout", "microsoft/phi-4-mini": "Phi-4 Mini"
}
...
SCRIPT_TEMPLATES = {
    'inline_bot': {
        'name': 'InlineKeyboard Bot', 'lang': 'py',
        'code': 'import os\nimport telebot\nfrom telebot import types\n\nbot = telebot.TeleBot(os.environ.get("BOT_TOKEN", ""))\n\n@bot.message_handler(commands=["start"])\ndef s(m):\n    mk = types.InlineKeyboardMarkup()\n    mk.add(types.InlineKeyboardButton("Ping", callback_data="ping"))\n    bot.reply_to(m, "Hello!", reply_markup=mk)\n\n@bot.callback_query_handler(func=lambda c: True)\ndef cb(c):\n    if c.data == "ping":\n        bot.answer_callback_query(c.id, "Pong!")\n\nbot.infinity_polling()\n'
    },
    'echo_bot': {
        'name': 'Echo Bot', 'lang': 'py',
        'code': 'import os\nimport telebot\n\nbot = telebot.TeleBot(os.environ.get("BOT_TOKEN", ""))\n\n@bot.message_handler(commands=["start"])\ndef s(m):\n    bot.reply_to(m, "Hello!")\n\n@bot.message_handler(func=lambda m: True)\ndef e(m):\n    bot.reply_to(m, f"You said: {m.text}")\n\nbot.infinity_polling()\n'
    },
    'web_scraper': {
        'name': 'Web Scraper', 'lang': 'py',
        'code': 'import requests\nimport time\nfrom datetime import datetime\n\nwhile True:\n    try:\n        r = requests.get("https://example.com", timeout=10)\n        print(f"[{datetime.now()}] Status: {r.status_code}")\n    except Exception as e:\n        print(f"Error: {e}")\n    time.sleep(60)\n'
    },
    'data_logger': {
        'name': 'Data Logger', 'lang': 'py',
        'code': 'import time\nimport json\nimport random\nfrom datetime import datetime\n\nwhile True:\n    entry = {"ts": datetime.now().isoformat(), "value": random.randint(1,100)}\n    with open("data_log.jsonl", "a") as f:\n        f.write(json.dumps(entry) + "\\n")\n    print(f"Logged: {entry}")\n    time.sleep(30)\n'
    },
    'cron_worker': {
        'name': 'Cron Worker', 'lang': 'py',
        'code': 'import time\nfrom datetime import datetime\n\nwhile True:\n    print(f"[{datetime.now()}] Working...")\n    time.sleep(3600)\n'
    },
}
...
def get_user_tier(uid):
    ...
    if uid in user_tiers: return user_tiers[uid]
...
def is_valid_python_code(code):
    """Check if the text is actually valid Python code (not AI thinking)."""
    if not code:
        return False
    if len(code) < 300:
        return False
    if not any(k in code for k in ('import ', 'def ', 'class ', 'print(', 'bot.')):
        return False
    reasoning_markers = [
        "we need to produce", "the user provided", "let's parse",
        "need careful", "we'll craft", "i'll write",
        "the user asks", "should provide", "need to follow",
        "the model identity", "we should", "let's implement",
        "parsing requirements", "checking requirements",
        "note that", "should we", "we can output",
    ]
    code_lower = code[:1500].lower()
    for marker in reasoning_markers:
        if marker in code_lower:
            return False
    first_line = code.split('\n', 1)[0].strip()
    if not (first_line.startswith(('import ', 'from ', '#', '"""', "'''", 'def ', 'class ', 'if ', '@'))
            or '=' in first_line):
        return False
    return True
...
def check_expired_subs():
    while True:
        try:
            time.sleep(3600)
            now = datetime.now()
            expired = [uid for uid, sub in list(user_subscriptions.items())
                       if sub.get('expiry') and sub['expiry'] < now]
            for uid in expired:
                try:
                    remove_subscription_db(uid)
                    bot.send_message(uid, "Your subscription expired. You are now on Free tier.")
                except: pass
        except Exception as e:
            ...
            for key in list(bot_scripts.keys()):
                ...
                if res and res['mem_mb'] > limits['ram_mb']:
                    ...
                    if info:
                        ...
                        try: bot.send_message(info['chat_id'], f"{info['file_name']} killed: RAM limit exceeded.")
                        except: pass
        except Exception as e:
...
# ==========================================================
#  === MISSING FUNCTION #1: save_user_file ===
# ==========================================================
def save_user_file(uid, fname, ftype='py'):
    """Save a file record to the database and in-memory cache."""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
...
# ==========================================================
#  === MISSING FUNCTION #2: remove_user_file_db ===
# ==========================================================
def remove_user_file_db(uid, fname):
    """Remove a file record from the database and in-memory cache."""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
...
# ==========================================================
#  === MISSING FUNCTION #3: remove_subscription_db ===
# ==========================================================
def remove_subscription_db(uid):
    """Remove a user's subscription and tier from DB and memory."""
    with DB_LOCK:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        try:
...
# ==========================================================
#  === MISSING FUNCTION #4: execute_broadcast ===
# ==========================================================
def execute_broadcast(txt, photo, video, cap, admin_chat):
    """
    Broadcast a message to all active users.
    Supports text, photo, or video with optional caption.
    Handles flood-wait, blocked users, and rate limiting.
    """
    ...
    if total == 0:
        try:
            bot.send_message(admin_chat, "Broadcast: no users to send to.")
        except: pass
        return
    ...
    for i, u in enumerate(users):
        try:
            if txt:
                ...
                bot.send_photo(u, photo, caption=cap or "")
            elif video:
                bot.send_video(u, video, caption=cap or "")
            sent += 1
        except telebot.apihelper.ApiTelegramException as e:
            ed = str(e).lower()
            if any(s in ed for s in ["blocked", "deactivated", "not found",
                                      "kicked", "restricted", "chat not found"]):
                blocked += 1
                active_users.discard(u)
            elif "flood" in ed or "too many" in ed or "retry" in ed:
        ...
        # Rate limiting: every 25 messages, pause 1.5s; every 5, pause 0.2s
...
def create_marketplace_menu():
    ...
    try:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        c.execute('SELECT id, name, language, downloads FROM marketplace ORDER BY downloads DESC LIMIT 20')
        for mid, name, lang, dl in c.fetchall():
            mk.add(types.InlineKeyboardButton(f"{name} ({lang}) | {dl} dl",
...
def _logic_my_tier(msg):
    ...
    text = (f"Your Tier: {tier['name']}{exp_info}\n\n"
            ...
            f"API Keys: {tier['api_keys_allowed']}")
...
def cb_buy_tier(call):
    ...
    try:
        ...
        bot.send_invoice(
            ...
            description=f"{tier['name']} tier for 30 days.",
...
def _logic_api_keys(msg):
    ...
    if is_prem:
        ...
        mk.row(types.InlineKeyboardButton("Create Key (Premium)", callback_data='apikey_create'))
    mk.row(types.InlineKeyboardButton("API Dashboard", url=f"{PUBLIC_URL}/dashboard"))
    mk.row(types.InlineKeyboardButton("API Health", url=f"{PUBLIC_URL}/health"))
    mk.row(types.InlineKeyboardButton("Back", callback_data='back_to_main'))
    if is_prem:
        bot.reply_to(msg, "API Key Management\n\nPremium tier active.", reply_markup=mk)
    else:
        bot.reply_to(msg, "API Key Management\n\nAPI keys are PREMIUM only.\nBuy a premium tier to unlock.", reply_markup=mk)


def cb_apikey_create(call):
    uid = call.from_user.id
    if not is_premium(uid):
        bot.answer_callback_query(call.id, "Premium only. Buy a premium tier to create API keys.", show_alert=True)
        return
    tier_key = get_user_tier(uid)
    tier_data = TIERS.get(tier_key, TIERS['free'])
    max_keys = tier_data.get('api_keys_allowed', 0)
    if max_keys == 0:
    ...
    if len(existing) >= max_keys:
        bot.answer_callback_query(call.id, f"Max {max_keys} API keys for your {tier_data['name']} tier.", show_alert=True)
        return
    bot.answer_callback_query(call.id)
    m = bot.send_message(
        uid,
        "Create Personal Bot via API\n\n"
        "I will generate a personal Telegram bot for you that can "
        "upload, start, stop, and manage scripts using the ATX API.\n\n"
        "Step 1/2: Send your BOT_TOKEN for the personal bot.\n"
        "(from @BotFather, must be a NEW bot, not the ATX Hosting itself)\n\n"
        "Format: 123456789:ABCdefGHI...\n\n"
        "/cancel to abort"
    )
    bot.register_next_step_handler(m, process_apikey_bot_token)


def process_apikey_bot_token(message):
    uid = message.from_user.id
    token = (message.text or "").strip()
    if token.lower() == '/cancel':
        bot.reply_to(message, "Cancelled.")
        return
    if token == PARENT_BOT_TOKEN:
        bot.reply_to(message, "Cannot use ATX Hosting's own BOT_TOKEN!\n\nCreate a NEW bot via @BotFather.")
        m = bot.send_message(uid, "Send BOT_TOKEN again:")
        bot.register_next_step_handler(m, process_apikey_bot_token)
        return
    if not re.match(r'^\d{8,12}:[A-Za-z0-9_-]{30,45}$', token):
        bot.reply_to(message, "Invalid format. Try again or /cancel.")
        m = bot.send_message(uid, "Send BOT_TOKEN again:")
        bot.register_next_step_handler(m, process_apikey_bot_token)
        return
    try:
        ...
        data = r.json()
        if not data.get('ok'):
            bot.reply_to(message, f"Invalid token. Telegram says: {data.get('description', 'unknown')}\n\nTry again.")
            m = bot.send_message(uid, "Send BOT_TOKEN again:")
            bot.register_next_step_handler(m, process_apikey_bot_token)
            return
        ...
        bot.register_next_step_handler(m, process_apikey_bot_token)
        return
    pending_api_key_requests[uid] = {'bot_token': token, 'bot_username': bot_username}
    m = bot.send_message(
        uid,
        f"Valid token!\nBot: @{bot_username}\n\n"
        f"Step 2/2: Send your ADMIN_ID (your Telegram ID).\n\n/cancel to abort"
    )
    bot.register_next_step_handler(m, process_apikey_admin_id)


def process_apikey_admin_id(message):
    uid = message.from_user.id
    admin_str = (message.text or "").strip()
    if admin_str.lower() == '/cancel':
        pending_api_key_requests.pop(uid, None)
        bot.reply_to(message, "Cancelled.")
        return
    try:
        ...
        bot.reply_to(message, "Invalid admin ID. Must be a number.")
        m = bot.send_message(uid, "Send ADMIN_ID again:")
        bot.register_next_step_handler(m, process_apikey_admin_id)
        return
    if uid not in pending_api_key_requests:
        bot.reply_to(message, "Session expired. Click Create Key again.")
        return
    pending_api_key_requests[uid]['admin_id'] = admin_id
    api_key = 'ATXhost_' + secrets.token_urlsafe(32)
    user_api_keys[api_key] = uid
    try:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        c.execute('INSERT INTO api_keys VALUES (?,?,?,?)',
                  (uid, api_key, datetime.now().isoformat(), None, 1))
        conn.commit()
        conn.close()
    except Exception as e:
        logger.error(f"api_keys insert: {e}")
    bot.reply_to(message, "All set!\n\nGenerating your personal bot...\nPlease wait (up to 90s)")
    bt = pending_api_key_requests[uid]
    threading.Thread(
        target=_generate_apikey_hosting_bot,
        args=(uid, api_key, bt['bot_token'], bt['admin_id'], bt['bot_username'], message)
    ).start()


def generate_fallback_hosting_bot(api_key, bot_token, admin_id, bot_username, owner_id, api_base):
    """Built-in template - guaranteed to work."""
    return f'''# Personal Script Management Bot
# Bot: @{bot_username}
# Admin ID: {admin_id}
# API Key: {api_key}
# API URL: {api_base}
...
ATX_API_KEY = os.environ.get('ATX_API_KEY', '{api_key}')
ATX_API_URL = os.environ.get('ATX_API_URL', '{api_base}')
OWNER_ID = {owner_id}

HEADERS = {{'X-API-Key': ATX_API_KEY}}
...
def main_menu():
    mk = types.InlineKeyboardMarkup(row_width=2)
    mk.row(
        types.InlineKeyboardButton("Upload Script", callback_data='m_upload'),
        types.InlineKeyboardButton("My Scripts", callback_data='m_files')
    )
    mk.row(
        types.InlineKeyboardButton("Statistics", callback_data='m_stats'),
        types.InlineKeyboardButton("My Usage", callback_data='m_usage')
    )
    mk.row(
        types.InlineKeyboardButton("My Plan", callback_data='m_tier'),
        types.InlineKeyboardButton("Dashboard", url=ATX_API_URL + '/dashboard')
    )
    return mk
...
def cmd_start(m):
    text = (
        ...
        f"Personal Script Management Bot\\n\\n"
        f"Commands:\\n"
        f"/start - Main menu\\n"
        f"/upload - Upload a script\\n"
        f"/files - View and manage scripts\\n"
        f"/stats - System stats\\n"
        f"/usage - Your usage\\n"
        f"/tier - Subscription info\\n"
    )
...
def cmd_upload(m):
    bot.reply_to(m, "Send me a .py, .js, or .zip file to upload.")


@bot.message_handler(commands=['files'])
def cmd_files(m):
    try:
        r = requests.get(f"{{ATX_API_URL}}/api/scripts", headers=HEADERS, timeout=30)
        data = r.json()
        if not data:
            bot.reply_to(m, "No scripts yet. Use /upload to add one.")
            return
        mk = types.InlineKeyboardMarkup(row_width=1)
        for s in data:
            status = "ON" if s.get('running') else "OFF"
            mk.add(types.InlineKeyboardButton(
                f"[{{status}}] {{s['file']}} ({{s['type']}})",
                callback_data=f"file_{{s['file']}}"
            ))
        bot.reply_to(m, "Your scripts:", reply_markup=mk)
    except Exception as e:
...
def cmd_stats(m):
    try:
        r = requests.get(f"{{ATX_API_URL}}/api/stats", headers=HEADERS, timeout=30)
        data = r.json()
        text = (
            f"System Statistics\\n\\n"
            f"Users: {{data['users']}}\\n"
            f"Running: {{data['running_scripts']}}\\n"
            f"Files: {{data['files']}}\\n"
            f"Subscriptions: {{data['subscriptions']}}"
        )
        bot.reply_to(m, text)
    except Exception as e:
        bot.reply_to(m, f"Error: {{e}}")
...
def cmd_usage(m):
    try:
        r = requests.get(f"{{ATX_API_URL}}/api/my/usage", headers=HEADERS, timeout=30)
        data = r.json()
        limits = data.get('limits', {{}})
        text = (
            f"Your Usage\\n\\n"
            f"Tier: {{data['tier']}}\\n"
            f"CPU: {{data['cpu_minutes']:.0f}} min\\n"
            f"AI Requests: {{data['ai_requests']}}/{{limits.get('ai_requests_day', '?')}}\\n"
...
def cmd_tier(m):
    try:
        r = requests.get(f"{{ATX_API_URL}}/api/my/tier", headers=HEADERS, timeout=30)
        data = r.json()
        lim = data['limits']
        text = (
            f"Your Plan: {{data['tier'].upper()}}\\n\\n"
            f"Scripts: {{lim['scripts']}}\\n"
            f"Max File: {{lim['file_mb']}} MB\\n"
            f"RAM: {{lim['ram_mb']}} MB\\n"
            f"API Rate: {{lim['api_rate']}}/min\\n"
            f"AI/Day: {{lim['ai_requests_day']}}\\n"
            f"API Keys: {{lim.get('api_keys_allowed', 0)}}"
        )
        if data.get('expires'):
...
def on_document(m):
    try:
        ...
        if ext not in ['.py', '.js', '.zip', '.ts', '.rb', '.go', '.php', '.sh']:
            bot.reply_to(m, f"Not supported: {{ext}}")
            return
        bot.reply_to(m, f"Uploading {{fname}}...")
        file_info = bot.get_file(doc.file_id)
        downloaded = bot.download_file(file_info.file_path)
        files = {{'file': (fname, BytesIO(downloaded))}}
        r = requests.post(f"{{ATX_API_URL}}/api/upload", files=files, headers=HEADERS, timeout=120)
        if r.status_code == 200:
            data = r.json()
            bot.reply_to(m, f"Uploaded: {{data['file']}} ({{data['type']}})\\n\\nUse /files to see it.")
        else:
            try:
                ...
                err = r.text[:200]
                reason = ''
            bot.reply_to(m, f"Upload failed: {{err}}\\n{{reason}}")
    except Exception as e:
        bot.reply_to(m, f"Error: {{e}}")


def show_file_panel(call, fname):
    try:
        r = requests.get(f"{{ATX_API_URL}}/api/scripts", headers=HEADERS, timeout=30)
        files = r.json()
        info = next((f for f in files if f['file'] == fname), None)
        if not info:
        ...
        if info['running']:
            ...
            mk.row(
                types.InlineKeyboardButton("Start", callback_data=f"start_{{fname}}"),
                types.InlineKeyboardButton("Logs", callback_data=f"logs_{{fname}}")
            )
        mk.add(types.InlineKeyboardButton("Delete", callback_data=f"delete_{{fname}}"))
        mk.add(types.InlineKeyboardButton("Back", callback_data='m_files'))
        try:
...
def on_callback(c):
    try:
        if c.data == 'm_upload':
            ...
            bot.send_message(c.message.chat.id, "Send me a .py, .js, or .zip file.")
        elif c.data == 'm_files':
            bot.answer_callback_query(c.id)
            cmd_files(c.message)
        elif c.data == 'm_stats':
            ...
            bot.answer_callback_query(c.id)
            cmd_tier(c.message)
        elif c.data.startswith('file_'):
            show_file_panel(c, c.data[5:])
        elif c.data.startswith('start_'):
            fname = c.data[6:]
            try:
                r = requests.post(f"{{ATX_API_URL}}/api/script/{{fname}}/start", headers=HEADERS, timeout=30)
                bot.answer_callback_query(c.id, "Started" if r.status_code == 200 else "Error")
                time.sleep(1)
                show_file_panel(c, fname)
            except Exception as e:
                bot.answer_callback_query(c.id, f"Error: {{e}}")
        elif c.data.startswith('stop_'):
            fname = c.data[5:]
            try:
                r = requests.post(f"{{ATX_API_URL}}/api/script/{{OWNER_ID}}/{{fname}}/stop", headers=HEADERS, timeout=30)
                bot.answer_callback_query(c.id, "Stopped" if r.status_code == 200 else "Error")
                time.sleep(1)
                show_file_panel(c, fname)
            except Exception as e:
                bot.answer_callback_query(c.id, f"Error: {{e}}")
        elif c.data.startswith('logs_'):
            fname = c.data[5:]
            try:
                r = requests.get(f"{{ATX_API_URL}}/api/script/{{fname}}/logs", headers=HEADERS, timeout=30)
                if r.status_code == 200:
                    logs = r.json().get('logs', '')
                    if len(logs) > 3500:
                        logs = "...\\n" + logs[-3500:]
                    bot.send_message(c.message.chat.id, f"Logs for {{fname}}:\\n{{logs}}")
                else:
                    bot.send_message(c.message.chat.id, "No logs available.")
                bot.answer_callback_query(c.id)
            except Exception as e:
                bot.answer_callback_query(c.id, f"Error: {{e}}")
            ...
            fname = c.data[7:]
            try:
                r = requests.post(f"{{ATX_API_URL}}/api/script/{{fname}}/delete", headers=HEADERS, timeout=30)
                bot.answer_callback_query(c.id, "Deleted" if r.status_code == 200 else "Error")
                cmd_files(c.message)
            except Exception as e:
                bot.answer_callback_query(c.id, f"Error: {{e}}")
    except Exception as e:
...
def _generate_apikey_hosting_bot(uid, api_key, bot_token, admin_id, bot_username, message):
    """Generate a personal bot via AI with SHORT direct prompt."""
    api_base = PUBLIC_URL

    prompt = f"""Write Python code. No explanation. No markdown.

Telegram bot for managing user's own scripts via ATX Hosting API.
...
API calls use header: {{'X-API-Key': ATX_API_KEY}}

Create bot with telebot and requests libraries.

Commands needed:
- /start: welcome + main_menu() with inline buttons
- /upload: instruct user to send file
- /files: list scripts (GET /api/scripts) with inline buttons per file
- /stats: show system stats (GET /api/stats)
- /usage: show usage (GET /api/my/usage)
- /tier: show tier (GET /api/my/tier)

Handle document uploads (.py/.js/.zip) via POST /api/upload with 'file' field.

Per-file buttons: Start (POST /api/script/<name>/start), Stop (POST /api/script/{uid}/<name>/stop), Logs (GET /api/script/<name>/logs), Delete (POST /api/script/<name>/delete).

Wrap all HTTP calls in try/except.

Main loop: bot.infinity_polling() wrapped in try/except with retry.

Only ADMIN_ID can use commands.

Start with "import" line. Output code only."""
    ...
    if res.get('error'):
        logger.warning(f"AI error for {uid}: {res['error']}. Using fallback.")
        use_fallback = True
    else:
        raw = res.get('answer', '')
        code = clean_ai_code(raw)
        if not is_valid_python_code(code):
            logger.warning(f"AI returned invalid/thinking response for {uid}. Using fallback.")
            use_fallback = True
        else:
            logger.info(f"AI generated valid code for {uid} ({len(code)} chars)")

    if use_fallback:
        code = generate_fallback_hosting_bot(api_key, bot_token, admin_id, bot_username, uid, api_base)
        try:
            bot.send_message(uid, "Using built-in template for your bot (AI response was not valid code).")
        except: pass
    ...
    folder = get_user_folder(uid)
    fname = f"hosting_bot_{int(time.time())}.py"
    fp = os.path.join(folder, fname)
    with open(fp, 'w', encoding='utf-8') as f:
        f.write(code)

    save_user_file(uid, fname, 'py')
    save_env_var(uid, fname, 'BOT_TOKEN', bot_token)
    save_env_var(uid, fname, 'ADMIN_ID', str(admin_id))
    save_env_var(uid, fname, 'ATX_API_KEY', api_key)
    save_env_var(uid, fname, 'ATX_API_URL', api_base)
    save_user_bot_token(uid, bot_token, admin_id)
    log_activity(uid, "hosting_bot_created", fname)
    ...
    mk = types.InlineKeyboardMarkup(row_width=2)
    mk.row(
        types.InlineKeyboardButton("Start Hosting Bot", callback_data=f'start_{uid}_{fname}'),
        types.InlineKeyboardButton("View Code", callback_data=f'download_{uid}_{fname}')
    )
    mk.row(
        types.InlineKeyboardButton("Check Files", callback_data='check_files'),
        types.InlineKeyboardButton("API Dashboard", url=f"{PUBLIC_URL}/dashboard")
    )
    ...
    caption = (
        ...
        f"INJECTED:\n"
        f"- BOT_TOKEN\n- ADMIN_ID\n- ATX_API_KEY\n- ATX_API_URL\n\n"
        f"Commands: /start /upload /files /stats /usage /tier\n\n"
        f"Click Start Hosting Bot to run."
    )

    try:
        with open(fp, 'rb') as f:
            bot.send_document(uid, f, caption=caption, parse_mode='Markdown', reply_markup=mk)
    except Exception as e:
        logger.error(f"send file: {e}")
        bot.send_message(uid, f"File generated: `{fname}`\n\nUse Check Files.", parse_mode='Markdown', reply_markup=mk)
...
def cb_ai_gen(call):
    ...
    if uid not in user_bot_tokens:
        m = bot.send_message(
            ...
            "Before generating, save:\n"
            "1. Bot Token (from @BotFather)\n"
            "2. Admin ID (your Telegram ID)\n\n"
...
def process_setup_bot_token(message):
    ...
    if not re.match(r'^\d+:[A-Za-z0-9_-]+$', token):
        bot.reply_to(message, "Invalid format. Try again or /cancel.")
        m = bot.send_message(message.chat.id, "Enter BOT TOKEN:")
        bot.register_next_step_handler(m, process_setup_bot_token)
        return
    m = bot.send_message(message.chat.id, "Bot token saved!\n\nEnter ADMIN ID:\n\n/cancel to abort")
...
def process_setup_admin_id(message, bot_token):
    ...
    try:
        ...
        m = bot.send_message(message.chat.id, "Enter ADMIN_ID:")
        bot.register_next_step_handler(m, process_setup_admin_id, bot_token)
        return
    save_user_bot_token(uid, bot_token, admin_id)
    bot.reply_to(message, f"Setup Complete!\n\nNow describe the bot:")
    m = bot.send_message(message.chat.id, "Example: 'telegram bot that replies to /start with hello'\n\n/cancel to abort")
    bot.register_next_step_handler(m, process_ai_gen)
...
def process_ai_gen(message):
    ...
    if not desc: return
    if uid not in user_bot_tokens:
        bot.reply_to(message, "No bot token saved. Click AI Generator again.")
        return
    ...
    prompt = f"""Write Python code. No explanation.

USER REQUEST: {desc}

USE THESE VALUES:
BOT_TOKEN = os.environ.get('BOT_TOKEN', '{bt['bot_token']}')
ADMIN_ID = os.environ.get('ADMIN_ID', '{bt['admin_id']}')

Use telebot library. Include InlineKeyboardMarkup. Only ADMIN_ID can use commands.
Wrap main loop in try/except. Start with "import" line. Output code only."""
...
def _generate_and_save(uid, desc, prompt, model, message):
    ...
    if res.get('error'):
        ...
        return
    code = clean_ai_code(res.get('answer', ''))
    if not is_valid_python_code(code):
        try: bot.send_message(message.chat.id, "AI returned invalid code. Try again.")
        except: pass
        return
    folder = get_user_folder(uid)
    ...
    try:
        bot.send_message(message.chat.id, f"Generated: {fname}\n\nPreview:\n{code[:1000]}", reply_markup=mk)
    except:
...
def _logic_batch(msg):
    ...
    bot.reply_to(msg, "Batch Selection:", reply_markup=create_batch_menu(uid))
...
def _logic_stats(msg):
    ...
    running = sum(1 for k, i in list(bot_scripts.items()) if is_bot_running(i['script_owner_id'], i['file_name']))
...
def _logic_ai_gen(msg):
    uid = msg.from_user.id
    if uid not in user_bot_tokens:
        m = bot.send_message(msg.chat.id, "AI Bot Generator Setup\n\nSend BOT TOKEN:")
        bot.register_next_step_handler(m, process_setup_bot_token)
        return
    if not check_ai_quota(uid):
...
def _logic_templates(msg):
    bot.reply_to(msg, "Templates:", reply_markup=create_templates_menu())
...
def _logic_daily(msg):
    ...
    tier = 'basic' if get_user_tier(uid) == 'free' else get_user_tier(uid)
...
def _logic_marketplace(msg):
    bot.reply_to(msg, "Marketplace:", reply_markup=create_marketplace_menu())
...
def _logic_api_dashboard(msg):
    text = (f"ATX Hosting API\n\nBase URL: {PUBLIC_URL}\n\n"
            f"Endpoints:\n- GET /health\n- GET /dashboard\n- GET /api/stats\n"
            f"- GET /api/scripts\n- GET /api/my/usage\n- GET /api/my/tier\n"
            f"- POST /api/upload\n- POST /api/script/<name>/start\n"
            f"- POST /api/script/<file>/stop\n- POST /api/script/<name>/delete\n"
            f"- GET /api/script/<name>/logs\n\n"
            f"Auth: X-API-Key header or ?api_key=\nGet your key: /apikeys")
    mk = types.InlineKeyboardMarkup(row_width=1)
...
def handle_doc(msg):
    ...
    if doc.file_size > max_size:
        bot.reply_to(msg, f"Max {limits['file_mb']} MB.")
        return
    try:
        ...
        if ext == '.py':
            has_conflict, reason = check_token_conflict(content, fn)
            if has_conflict:
                bot.edit_message_text(f"Token conflict:\n\n{reason}", cid, w.message_id)
                return
        bot.edit_message_text(f"Processing...", cid, w.message_id)
    ...
    try:
        ...
        with zipfile.ZipFile(zp, 'r') as zr:
            for m in zr.infolist():
                if m.filename.endswith('.py'):
                    try:
                        file_content = zr.read(m.filename)
                        has_conflict, reason = check_token_conflict(file_content, m.filename)
                        if has_conflict:
                            bot.reply_to(msg, f"Token conflict in ZIP:\n\n{reason}")
                            return
                    except: pass
        ...
        if pkg:
            try:
                ...
                return
        if not scripts:
            bot.reply_to(msg, "No script found!")
            return
...
def handle_cb(call):
    ...
    if not check_rate_limit(uid, limit=60, window=60):
        bot.answer_callback_query(call.id, "Rate limit.", show_alert=True)
        return
    try:
...
def cb_start(call):
    try:
        ...
        r = is_bot_running(soid, fn)
        try:
            bot.edit_message_text(f"Controls for: {fn} ({ft})\nStatus: {'Running' if r else 'Starting...'}",
                                  call.message.chat.id, call.message.message_id,
                                  reply_markup=create_control_buttons(soid, fn, r))
        except: pass
    except Exception as e:
...
def cb_backmain(call):
    uid = call.from_user.id
    text = f"Welcome back, {call.from_user.first_name}!\n\nID: {uid}\nTier: {TIERS[get_user_tier(uid)]['name']}"
    try:
        bot.answer_callback_query(call.id)
...
def cb_aimenu(call):
    bot.answer_callback_query(call.id)
    try:
        bot.edit_message_text("AI Assistant:", call.message.chat.id, call.message.message_id,
    ...
    if match:
        ...
        if os.path.exists(lp):
            try:
                ...
                q = f"Logs of my script. Find error. No think tags.\n\nScript: {match[0]}\n\nLogs:\n{content}"
            except: pass
    ai_seek_send_long(m.chat.id, get_user_ai_model(uid), q, m.message_id)
...
def cb_aisetmodel(call):
    ...
    if model in AI_SEEK_MODELS:
        ...
        try:
            bot.edit_message_text(f"Model: {AI_SEEK_MODEL_LABELS.get(model, model)}",
                                  call.message.chat.id, call.message.message_id,
                                  reply_markup=create_ai_menu(uid))
        except: pass
...
def cb_mktitem(call):
    try:
        ...
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        c.execute('SELECT id, uploader_id, name, description, language, file_path, downloads FROM marketplace WHERE id=?', (mid,))
        row = c.fetchone()
        if not row:
            bot.answer_callback_query(call.id, "Not found.", show_alert=True)
            conn.close()
            return
        _, uploader, name, desc, lang, fpath, dl = row
        c.execute('UPDATE marketplace SET downloads=downloads+1 WHERE id=?', (mid,))
        conn.commit()
        conn.close()
        mk = types.InlineKeyboardMarkup(row_width=1)
        mk.add(types.InlineKeyboardButton("Download", callback_data=f'mkt_dl_{mid}'))
        text = (f"{name}\nLanguage: {lang}\nUploader: {uploader}\n"
                f"Downloads: {dl+1}\n\nDesc:\n{desc}")
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id, text, reply_markup=mk)
...
def cb_mkt_dl(call):
    try:
        ...
        bot.send_message(uid, f"{name} downloaded.")
    except Exception as e:
        logger.error(f"mkt_dl: {e}")


def cb_mkt_rate(call):
    bot.answer_callback_query(call.id, "Rating disabled.")
...
def cb_mkt_reviews(call):
    bot.answer_callback_query(call.id, "No reviews.")
...
def proc_cicd_setup(message, soid, fn):
    ...
    try:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        c.execute('INSERT OR REPLACE INTO ci_cd VALUES (?,?,?,?,?,?)', (soid, fn, url, branch, 1, None))
        conn.commit()
        conn.close()
...
def cb_sandbox(call):
    try:
        ...
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id,
                         f"Sandbox Info for {fn}\n\nStatus: {'ENABLED' if SANDBOX_MODE else 'DISABLED'}")
    except: pass
...
def cb_batch_open(call):
    ...
    bot.answer_callback_query(call.id)
    try:
        bot.edit_message_text("Batch Selection:", call.message.chat.id, call.message.message_id,
...
def health():
    return jsonify({
        "status": "ok", "service": "ATX Hosting", "version": "7.1",
        "running_scripts": len(bot_scripts), "active_users": len(active_users),
...
def api_scripts():
    ...
    for fn, ft in user_files.get(uid, []):
        if not any(d['file'] == fn for d in data):
            data.append({
                "key": f"{uid}_{fn}", "owner": uid, "file": fn, "type": ft,
                "uptime": 0, "running": False
            })
    return jsonify(data)
...
def api_my_tier():
    ...
    if uid in user_subscriptions:
        e = user_subscriptions[uid].get('expiry')
        if e: exp = e.isoformat()
    return jsonify({"tier": tier_key, "limits": TIERS[tier_key], "expires": exp})
...
def api_upload():
    ...
    ext = os.path.splitext(fname)[1].lower()
    if ext not in SUPPORTED_EXTS:
        return jsonify({"error": f"allowed: {', '.join(SUPPORTED_EXTS)}"}), 400
    limits = get_tier_limits(uid)
    if get_user_file_count(uid) >= limits['scripts']:
        return jsonify({"error": f"limit reached ({limits['scripts']})"}), 403
    ufolder = get_user_folder(uid)
    fp = os.path.join(ufolder, fname)
    file.save(fp)
    if ext == '.py':
        try:
            with open(fp, 'rb') as f:
                content = f.read()
            has_conflict, reason = check_token_conflict(content, fname)
            if has_conflict:
                os.remove(fp)
                return jsonify({"error": "token conflict", "reason": reason}), 400
        except: pass
    ftype = LANG_CONFIG.get(ext, {}).get('type', 'py')
    save_user_file(uid, fname, ftype)
    log_activity(uid, "api_upload", fname)
    return jsonify({"status": "uploaded", "file": fname, "type": ftype})


@app.route('/api/script/<file_name>/start', methods=['POST'])
def api_start(file_name):
    uid = check_api_auth()
    if not uid: return jsonify({"error": "unauthorized"}), 401
    if file_name not in [f[0] for f in user_files.get(uid, [])]:
    ...
    ftype = next((f[1] for f in user_files.get(uid, []) if f[0] == file_name), 'py')
    dummy = type('Msg', (), {
        'chat': type('C', (), {'id': uid})(),
        'message_id': None,
        'from_user': type('U', (), {'id': uid})()
    ...
    if ftype == 'py':
        threading.Thread(target=run_script, args=(fp, uid, folder, file_name, dummy), daemon=True).start()
    elif ftype == 'js':
        threading.Thread(target=run_js_script, args=(fp, uid, folder, file_name, dummy), daemon=True).start()
    else:
        threading.Thread(target=run_generic_script, args=(fp, uid, folder, file_name, ftype, dummy), daemon=True).start()
    time.sleep(1.5)
    return jsonify({"status": "started", "running": is_bot_running(uid, file_name)})
...
@app.route('/api/script/<file_name>/logs', methods=['GET'])
def api_logs(file_name):
    uid = check_api_auth()
    if not uid: return jsonify({"error": "unauthorized"}), 401
    if file_name not in [f[0] for f in user_files.get(uid, [])]:
        return jsonify({"error": "not found"}), 404
    ...
    if len(content) > 10000:
        content = "...\n" + content[-10000:]
    return jsonify({"file": file_name, "logs": content})


@app.route('/api/script/<file_name>/delete', methods=['POST'])
def api_delete_script(file_name):
    uid = check_api_auth()
    if not uid: return jsonify({"error": "unauthorized"}), 401
    if file_name not in [f[0] for f in user_files.get(uid, [])]:
        return jsonify({"error": "not found"}), 404
    ...
    log_activity(uid, "api_delete", file_name)
    return jsonify({"status": "deleted", "file": file_name})


@app.route('/api/script/<file_name>/download', methods=['GET'])
def api_download(file_name):
    uid = check_api_auth()
    if not uid: return jsonify({"error": "unauthorized"}), 401
    if file_name not in [f[0] for f in user_files.get(uid, [])]:
        return jsonify({"error": "not found"}), 404
    ...
    return send_file(fp, as_attachment=True, download_name=file_name)
