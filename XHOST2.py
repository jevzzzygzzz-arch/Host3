# -*- coding: utf-8 -*-
"""
XHOST Bot v5.0 - Advanced Edition (No 2FA)
Features: Multi-File Batch, Per-User Quotas, Live Graphs, Sandbox,
Multi-Server Cluster, Marketplace, AI Generator, AI Auto-Fix,
CI/CD Pipeline, API Keys, Usage Billing, Subscription Tiers,
Telegram Stars, Sponsored Scripts, Rate Limiting, Content Moderation,
IP Whitelisting, K8s ready
"""
import telebot
import subprocess
import os
...
# ==========================================================
SCRIPT_TEMPLATES = {
    'echo_bot': {'name': 'Echo Bot (Python)', 'lang': 'py',
    ...
    'web_scraper': {'name': 'Web Scraper (Python)', 'lang': 'py',
                    'code': 'import requests, time\nfrom bs4 import BeautifulSoup\nwhile True:\n    try:\n        r = requests.get("https://example.com", timeout=10)\n        soup = BeautifulSoup(r.text, "html.parser")\n        print(f"Title: {soup.title.string if soup.title else \'N/A\'}")\n    except Exception as e: print(f"Err: {e}")\n    time.sleep(60)\n'},
    'data_logger': {'name': 'Data Logger (Python)', 'lang': 'py',
    ...
    'cron_worker': {'name': 'Cron Worker (Python)', 'lang': 'py',
...
def scan_file_for_malware(content, name, uid):
    ...
    if s:
        ...
        try: bot.send_message(OWNER_ID, f"Content flagged:\nUser: {uid}\nFile: {name}\nReason: {r}")
        except: pass
        return False, r
    return True, "safe"
...
def cb_sandbox(call):
    try:
        ...
        bot.send_message(call.message.chat.id,
                         ...
                         f"Sandbox applies CPU priority and RAM limits.")
    except Exception as e:
        logger.error(f"sandbox: {e}")
...
@app.route('/')
def home(): return "XHOST Bot v5.0"
...
def cicd_webhook():
    ...
    if not repo_url: return jsonify({"error": "invalid"}), 400
    try:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        ...
        for uid, fname in matches:
            ...
            if r.returncode == 0:
                ...
                if os.path.exists(new_fp):
                    ...
                    try: bot.send_message(uid, f"Auto-deployed {fname} from GitHub ({branch}).")
                    except: pass
...
def _logic_send_welcome(msg):
    ...
    text = (f"Welcome, {name}!\n\nID: {uid}\nUsername: @{uname or 'Not set'}\n"
            f"Tier: {tier['name']}\nFiles: {cur}/{tier['scripts']}\n\n"
    ...
    try: bot.send_message(cid, text, reply_markup=create_reply_keyboard_main_menu(uid))
...
def _logic_ai_gen(msg):
    ...
    if not check_ai_quota(uid):
        ...
        return
    m = bot.send_message(msg.chat.id, "AI Script Generator\n\nDescribe what you want:\n/cancel to abort.")
    bot.register_next_step_handler(m, process_ai_gen)
...
def cb_health(call):
    try:
        ...
        m = bot.send_message(call.message.chat.id, f"Health URL for {fn}\n\nCurrent: {cur}\n\nSend URL.\n/cancel to remove.")
...
def cb_collab(call):
    try:
        ...
        bot.answer_callback_query(call.id)
        m = bot.send_message(call.message.chat.id, "Send user ID:\n/cancel to abort.")
    ...
    if match:
        ...
        if os.path.exists(lp):
            try:
                ...
                q = f"Logs of script. Find error and explain:\n\n{content}"
            except: pass
    ai_seek_send_long(m.chat.id, get_user_ai_model(uid), q, m.message_id)
...
def cb_addadmin(call):
    bot.answer_callback_query(call.id)
    m = bot.send_message(call.message.chat.id, "User ID:\n/cancel to abort.")
