...
<p style="margin-top:30px;color:#444;font-size:0.85em;text-align:center">ATX Hosting - Powered by AI Seek</p>
</div></body></html>
"""
...
# Public URL for API link button - HARDCODED
...
# ==========================================================
#  TIERS - CUSTOM PRICING (50, 100, 200, 300 stars)
# ==========================================================
TIERS = {
...
# Store user bot tokens for AI-generated bots
user_bot_tokens = {}   # {user_id: {'bot_token': ..., 'admin_id': ...}}
...
# ==========================================================
#  DETECT VIRTUALENV (fix for --user install error)
# ==========================================================
def is_in_venv():
...
def get_pip_install_cmd(packages):
    """Get the correct pip install command depending on venv."""
    base_cmd = [sys.executable, '-m', 'pip', 'install']
    if not is_in_venv():
        # Only use --user when NOT in a venv
...
# ==========================================================
#  PACKAGE INSTALL (FIXED for venv)
# ==========================================================
TELEGRAM_MODULES = {
...
# ==========================================================
#  MENUS - NAVIGATION (like image)
# ==========================================================
def create_main_menu_inline(uid):
    """Main inline menu with navigation."""
    mk = types.InlineKeyboardMarkup(row_width=2)
    ...
    # API Dashboard link button (hardcoded URL)
...
def create_reply_keyboard_main_menu(uid):
    """Reply keyboard (bottom buttons) - navigation style."""
    layout = [
...
def cb_ai_gen(call):
    bot.answer_callback_query(call.id)
    uid = call.from_user.id
    # Check if user has saved bot token
    if uid not in user_bot_tokens:
        ...
        return
    # User already has token - ask for description
    m = bot.send_message(
        call.message.chat.id,
        "AI Script Generator\n\n"
...
def _generate_and_save(uid, desc, prompt, model, message):
    ...
    # Auto-save the bot token as env var for this script
