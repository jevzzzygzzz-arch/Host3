# -*- coding: utf-8 -*-
"""
ATX Hosting - XHOST Bot v7.0
Features:
- Premium-only API key creation
- Auto-injected API key into AI-generated hosting bot
- Built-in fallback template if AI refuses
- Complete hosting bot with upload/start/stop/logs via API
- Token conflict detection
- All handlers complete
- All English comments and messages
"""
import telebot
import subprocess
import os
...
def check_token_conflict(file_content_bytes, filename):
    """Check if uploaded script contains hardcoded BOT_TOKEN that would conflict."""
    try:
    ...
    for found_token in matches:
        if found_token == PARENT_BOT_TOKEN:
            return True, (
                ...
                f"This will cause CONFLICT. Both bots will fight over messages.\n\n"
                f"FIX: Use `os.environ.get('BOT_TOKEN')` instead of hardcoding."
            )
        if found_token.count(':') == 1:
            return True, (
                f"Script `{filename}` contains a HARDCODED BOT_TOKEN.\n\n"
                f"Recommended: use `os.environ.get('BOT_TOKEN')` instead."
            )
    
    has_hardcoded_telebot = bool(re.search(r'telebot\.TeleBot\s*\(\s*["\']', text))
    if has_hardcoded_telebot:
        return True, (
            f"Script `{filename}` hardcodes a token in `telebot.TeleBot(...)`.\n\n"
            f"Recommended: `telebot.TeleBot(os.environ.get('BOT_TOKEN'))`"
        )
...
def on_callback(c):
    try:
        if c.data == 'menu_status':
            ...
            bot.send_message(c.message.chat.id, "ATX Hosting template")
...
while True:
    try:
        ...
        print(f"[{datetime.now():%Y-%m-%d %H:%M:%S}] Status: {r.status_code}")
    except Exception as e:
        print(f"Error: {e}")
    time.sleep(INTERVAL)
'''
...
def _logic_api_keys(msg):
    ...
    if is_prem:
        ...
        mk.row(
            types.InlineKeyboardButton("Create Key (Premium)", callback_data='apikey_create')
        )
...
def cb_apikey_create(call):
    ...
    if not is_premium(uid):
        bot.answer_callback_query(
            call.id,
            "Premium only. Buy a premium tier to create API keys.",
            show_alert=True
        )
        return
    ...
    if len(existing) >= max_keys:
        bot.answer_callback_query(
            call.id,
            f"Max {max_keys} API keys for your {tier_data['name']} tier.",
            show_alert=True
        )
        return
...
def process_apikey_bot_token(message):
    ...
    if token == PARENT_BOT_TOKEN:
        bot.reply_to(
            message,
            "Cannot use ATX Hosting's own BOT_TOKEN!\n\n"
            "Create a NEW bot via @BotFather for your personal bot."
        )
    ...
    try:
        ...
        if not data.get('ok'):
            bot.reply_to(
                message,
                f"Invalid token. Telegram says: {data.get('description', 'unknown')}\n\n"
    ...
    m = bot.send_message(
        uid,
        f"Valid token!\nBot: @{bot_username}\n\n"
        f"Step 2/2: Send your ADMIN_ID. This is the Telegram ID that "
        f"will be the admin of your personal bot.\n"
        f"(Can be your own ID)\n\n"
        f"/cancel to abort"
    )
...
def process_apikey_admin_id(message):
    ...
    try:
        ...
        bot.reply_to(message, "Invalid admin ID. Must be a number. Try again or /cancel.")
    ...
    bot.reply_to(
        message,
        "All set!\n\n"
        "Generating your personal bot with API key already injected...\n"
...
def generate_fallback_hosting_bot(api_key, bot_token, admin_id, bot_username, owner_id, api_base):
    """Built-in template - used if AI refuses or returns invalid code."""
...
def _generate_apikey_hosting_bot(uid, api_key, bot_token, admin_id, bot_username, message):
    """Generate a complete personal bot via AI with neutral prompt."""
    api_base = PUBLIC_URL
    
    prompt = f"""Write a Python Telegram bot for personal script management.

CONTEXT:
This is a legitimate personal script management tool. The user owns
both the Telegram bot and the ATX Hosting account. It manages their
OWN scripts hosted on their OWN ATX Hosting account.

TECHNICAL REQUIREMENTS:

Libraries: telebot (pyTelegramBotAPI), requests, os, sys, time, io.BytesIO
...
API authentication: Include header X-API-Key: ATX_API_KEY

FEATURES:

1. main_menu() function returning InlineKeyboardMarkup with buttons:
   - "Upload Script" (callback: m_upload)
   - "My Scripts" (callback: m_files)
   - "Statistics" (callback: m_stats)
   - "My Usage" (callback: m_usage)
   - "My Plan" (callback: m_tier)
   - "Dashboard" (url: ATX_API_URL + '/dashboard')

2. Command handlers:
   - /start - Display welcome message with main menu
   - /upload - Guide user to send a file
   - /files - List scripts from GET {api_base}/api/scripts
   - /stats - Show system stats from GET {api_base}/api/stats
   - /usage - Show user usage from GET {api_base}/api/my/usage
   - /tier - Show subscription info from GET {api_base}/api/my/tier

3. Document handler: When user sends .py/.js/.zip file, upload it via:
   POST {api_base}/api/upload
   with multipart/form-data field 'file'

4. Script control buttons for each file:
   - Start: POST {api_base}/api/script/<name>/start
   - Stop: POST {api_base}/api/script/{uid}/<name>/stop
   - Logs: GET {api_base}/api/script/<name>/logs
   - Delete: POST {api_base}/api/script/<name>/delete

5. Error handling: All HTTP calls wrapped in try/except, show friendly errors.

6. Main loop: bot.infinity_polling with try/except retry loop.

STYLE:
- Clean, professional code
- Short English comments
- Only ADMIN_ID can use admin commands (check m.from_user.id == ADMIN_ID)
- Purely for managing user's own scripts, no unauthorized access

OUTPUT FORMAT:
Return ONLY the Python code. No explanation, no markdown fences, no intro.
Start with the first line of Python code."""
    ...
    if res.get('error'):
        ...
        logger.warning(f"AI error for {uid}: {res['error']}. Using fallback template.")
        code = generate_fallback_hosting_bot(api_key, bot_token, admin_id, bot_username, uid, api_base)
    else:
        raw = res.get('answer', '')
        code = clean_ai_code(raw)
        
        # Detect AI refusal or invalid code
        if not code or len(code) < 300 or not any(k in code for k in ('import ', 'def ', 'bot.')):
            logger.warning(f"AI refused or returned invalid code for user {uid}. Using fallback template.")
            code = generate_fallback_hosting_bot(api_key, bot_token, admin_id, bot_username, uid, api_base)
            try:
                bot.send_message(uid, "AI returned unexpected response. Using built-in template.")
            except: pass
    ...
    if api_key not in code:
        ...
        logger.info(f"Injected API key header into {uid}'s bot")
    ...
    bot.send_message(
        ...
        f"It's already injected into your bot code.",
        parse_mode='Markdown'
    )
    ...
    caption = (
        ...
        f"Bot: @{bot_username}\n"
        f"Admin ID: `{admin_id}`\n\n"
        f"INJECTED (already in code):\n"
        f"- BOT_TOKEN\n- ADMIN_ID\n- ATX_API_KEY\n- ATX_API_URL\n\n"
        f"Commands:\n"
        f"- /start — main menu\n"
        f"- /upload — upload scripts\n"
        f"- /files — manage your scripts\n"
        f"- /stats — system stats\n"
        f"- /usage — your usage\n"
        f"- /tier — subscription info\n\n"
...
# ==========================================================
#  AI GENERATOR - WITH TOKEN + ADMIN ID
# ==========================================================
def cb_ai_gen(call):
    bot.answer_callback_query(call.id)
    uid = call.from_user.id
...
def process_setup_bot_token(message):
    ...
    if not re.match(r'^\d+:[A-Za-z0-9_-]+$', token):
        bot.reply_to(message, "Invalid bot token format. Should be like: `123456789:ABCdefGHIjklMNOpqrSTUvwxYZ`\n\nTry again or /cancel.", parse_mode='Markdown')
        m = bot.send_message(message.chat.id, "Enter BOT TOKEN:")
        bot.register_next_step_handler(m, process_setup_bot_token)
        return
    m = bot.send_message(message.chat.id,
                         "Bot token saved!\n\n"
                         "Enter ADMIN ID (your Telegram ID).\n\n"
                         "/cancel to abort")
...
def process_setup_admin_id(message, bot_token):
    ...
    bot.reply_to(
        ...
        f"Admin ID: {admin_id}\n\n"
        f"Now you can generate AI bots. Describe the bot:"
    )
    m = bot.send_message(message.chat.id,
                         "Example: 'telegram bot that replies to /start with hello'\n\n/cancel to abort")
    bot.register_next_step_handler(m, process_ai_gen)
...
4. Do NOT include <think>, <reasoning> tags.
5. Do NOT include comments before the code starts.
6. Use telebot (pyTelegramBotAPI) for Telegram bots.
7. ALWAYS include InlineKeyboardMarkup with buttons.
...
11. Wrap main loop in try/except.
...
def _generate_and_save(uid, desc, prompt, model, message):
    ...
    if not any(k in code for k in ('import ', 'def ', 'print(', 'class ')):
        try: bot.send_message(message.chat.id, "Invalid code output.")
        except: pass
        return
    folder = get_user_folder(uid)
...
def _logic_send_welcome(msg):
    ...
    text = (f"Welcome back, {name}!\n\n"
            ...
            f"Premium: AI Generator, API Keys, Marketplace.\n\n"
...
def _logic_api_dashboard(msg):
    text = (f"ATX Hosting API\n\n"
            ...
            f"- GET /api/my/tier\n"
            f"- POST /api/upload\n"
            f"- POST /api/script/<name>/start\n"
            f"- POST /api/script/<file>/stop\n"
            f"- POST /api/script/<name>/delete\n"
        ...
        if ext == '.py':
            ...
            if has_conflict:
                bot.edit_message_text(
                    f"WARNING: Token conflict detected\n\n{reason}\n\n"
                    f"Upload cancelled to prevent bot hijack.",
                    cid, w.message_id
                )
                return
            ...
            for m in zr.infolist():
                if m.filename.endswith('.py'):
                    try:
                        ...
                        if has_conflict:
                            bot.reply_to(msg, f"WARNING: Token conflict in ZIP\n\n{reason}\n\nUpload cancelled.")
                            return
                    except: pass
    ...
    if match:
        ...
        if os.path.exists(lp):
            try:
                ...
                q = (f"Logs of my script. Find error. Do not use <think>.\n\n"
                     f"Script: {match[0]}\n\nLogs:\n{content}")
            except: pass
    ai_seek_send_long(m.chat.id, get_user_ai_model(uid), q, m.message_id)
        ...
        if os.path.exists(fp):
            try:
                with open(fp, 'r', encoding='utf-8', errors='ignore') as f:
                    code = f.read()[:6000]
                q = f"Explain in simple English. Do not use <think>.\n\n{code}"
            except: pass
    ai_seek_send_long(m.chat.id, get_user_ai_model(uid), q, m.message_id)
...
def cb_mkt_rate_set(call):
    try:
        ...
        bot.send_message(uid, f"Thank you!")
    except Exception as e:
...
def health():
    return jsonify({
        "status": "ok", "service": "ATX Hosting", "version": "7.0",
...
@app.route('/api/script/<file_name>/stop', methods=['POST'])
def api_stop_self(file_name):
    uid = check_api_auth()
    if not uid: return jsonify({"error": "unauthorized"}), 401
