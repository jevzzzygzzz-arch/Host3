...
def _generate_apikey_hosting_bot(uid, api_key, bot_token, admin_id, bot_username, message):
    """
    Generate a complete hosting bot via AI.
    API key is INJECTED into the code as default fallback.
    """
    api_base = PUBLIC_URL
    
    prompt = f"""Generate a COMPLETE Python Telegram hosting bot using ATX Hosting API.

This bot allows the user to UPLOAD, HOST, and MANAGE scripts via the ATX API.

CRITICAL RULES:
1. Output ONLY Python code. No explanation, no markdown fences.
2. Use `telebot` (pyTelegramBotAPI) and `requests`.
3. Read config from env vars WITH hardcoded fallback:
...
4. The API key injected is: {api_key}
   The API URL is: {api_base}
   The owner_id for stop endpoint is: {uid}
   DO NOT change these values.
5. Only ADMIN_ID can use admin commands.
6. Required commands (with InlineKeyboardMarkup menus):
   /start - welcome with main inline menu
   /upload - prompt for .py/.js/.zip file, upload via POST /api/upload (multipart with 'file' field)
   /files - list files via GET /api/scripts with inline buttons
   /start_file <name> - POST /api/script/<name>/start
   /stop_file <name> - POST /api/script/{uid}/<name>/stop
   /logs <name> - GET /api/script/<name>/logs
   /delete <name> - POST /api/script/<name>/delete
   /stats - GET /api/stats
   /usage - GET /api/my/usage
   /tier - GET /api/my/tier
7. Use requests for ALL API calls.
8. Always include header: {{'X-API-Key': ATX_API_KEY}}
9. On file upload: use multipart/form-data with 'file' field and BytesIO.
10. Show API errors nicely to user (no crashes).
11. Use InlineKeyboardMarkup for all menus.
12. File Control Panel: Start / Stop / Logs / Delete buttons.
13. Wrap all API calls in try/except.
14. Wrap main polling in try/except with auto-restart loop (sleep 5s on error).
15. Add document handler: on file received, upload via POST /api/upload.
16. Add short inline comments in Tagalog/English.
17. Must have function `main_menu()` that returns InlineKeyboardMarkup.

The bot should feel like a mini version of ATX Hosting.
...
Start your response with the first line of Python code. Nothing before it."""
    ...
    if api_key not in code:
        ...
        logger.info(f"Injected API key header into {uid}'s hosting bot")
    ...
    # Send API key separately
    bot.send_message(
        uid,
        f"YOUR API KEY\n\n`{api_key}`\n\n"
        f"Save this now — hindi na maipapakita muli.\n"
        f"It's already injected into your hosting bot code.",
        parse_mode='Markdown'
    )
    ...
    caption = (
        ...
        f"• BOT_TOKEN\n• ADMIN_ID\n• ATX_API_KEY\n• ATX_API_URL\n\n"
        f"Features:\n"
        f"• /upload — upload scripts via API\n"
        f"• /files — list & control scripts\n"
        f"• /start_file — start script\n"
        f"• /stop_file — stop script\n"
        f"• /logs — get logs\n"
        f"• /stats — bot stats\n"
        f"• /usage — your usage\n"
        f"• /tier — your tier info\n\n"
        f"Click Start Hosting Bot — ready na lahat."
    )
