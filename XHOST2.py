# -*- coding: utf-8 -*-
"""
ATX Hosting - XHOST Bot v6.2 (Token Conflict Fix)
Fixes:
- Auto-detect hardcoded BOT_TOKEN in uploaded scripts
- Block scripts that would conflict with parent bot
- Warn users about token issues
...
# ==========================================================
#  TOKEN CONFLICT DETECTION (NEW - IMPORTANT!)
# ==========================================================
PARENT_BOT_TOKEN = TOKEN  # Save the parent token for comparison
PARENT_BOT_TOKEN_SUFFIX = TOKEN.split(':')[-1][-10:] if ':' in TOKEN else ''
...
def check_token_conflict(file_content_bytes, filename):
    ...
    # Look for common patterns of hardcoded tokens
    # Pattern: quotes around 123456:ABC-xxx format
    token_pattern = r'["\'](\d{8,12}:[A-Za-z0-9_-]{30,45})["\']'
    matches = re.findall(token_pattern, text)
    
    for found_token in matches:
        # Check if it's the same as parent token
        if found_token == PARENT_BOT_TOKEN:
            return True, (
                ...
                f"or use a DIFFERENT bot token."
            )
        # Check if token looks like a valid format but different
        if found_token.count(':') == 1:
            # Different token — warning only, not block
            logger.warning(f"Script {filename} has hardcoded token: {found_token[:15]}...")
            return True, (
                f"Script `{filename}` contains a HARDCODED BOT_TOKEN.\n\n"
                f"Recommended: use `os.environ.get('BOT_TOKEN')` so you can change it "
                f"via Env Vars without re-uploading."
            )
    
    # Check for polling function usage without env var
...
def check_suspicious_keywords_in_script(file_content_bytes, filename):
    """
    Light check (not aggressive) - just warn on highly suspicious patterns.
    Does NOT block legitimate AES/crypto libraries.
    """
    try:
        text = file_content_bytes.decode('utf-8', errors='ignore')
    except:
    ...
    # Only flag if BOTH hardcoded token AND auto-polling combined
    has_token = bool(re.search(r'["\']\d{8,12}:[A-Za-z0-9_-]{30,45}["\']', text))
    has_polling = ('infinity_polling' in text or 
                   '.polling(' in text or 
                   'bot.polling' in text)
    
    if has_token and has_polling:
        return True, (
            f"Script `{filename}` has hardcoded token + auto-polling.\n\n"
            f"This means when you start it, it will compete with ATX Hosting bot "
            f"for Telegram messages.\n\n"
            f"FIX: use `os.environ.get('BOT_TOKEN')` and provide the token via Env Vars."
        )
...
def process_setup_bot_token(message):
    ...
    m = bot.send_message(message.chat.id,
                         ...
                         "if you are a admin enter your id.\n\n"
                         "/cancel to abort")
...
def process_setup_admin_id(message, bot_token):
    ...
    try:
        ...
        m = bot.send_message(message.chat.id, "Enter ADMIN ID:")
...
def _logic_ai_gen(msg):
    ...
    if uid not in user_bot_tokens:
        m = bot.send_message(msg.chat.id,
                             "AI Bot Generator Setup\n\n"
                             "Before you can generate, you need to save:\n"
                             "1. Bot Token (mula kay @BotFather)\n"
                             "2. Admin ID\n\n"
                             "Enter BOT TOKEN:")
        bot.register_next_step_handler(m, process_setup_bot_token)
        ...
        if ext == '.py':
            ...
            if has_conflict:
                bot.edit_message_text(
                    ...
                    f"To proceed, use os.environ.get('BOT_TOKEN') and provide token via Env Vars.\n\n"
...
def handle_zip_file(content, fname_zip, msg):
    ...
    try:
        ...
        with zipfile.ZipFile(zp, 'r') as zr:
            # CHECK FOR TOKEN CONFLICT IN ANY .PY FILE
            for m in zr.infolist():
                if m.filename.endswith('.py'):
                    try:
                        ...
                        if has_conflict:
                            bot.reply_to(msg, 
                                f"WARNING: Token conflict detected in ZIP\n\n{reason}\n\n"
                                f"Upload cancelled.")
                            return
                    except: pass
...
if __name__ == '__main__':
    logger.info("=" * 50)
    logger.info("ATX Hosting v6.2 (Token Conflict Fix)")
