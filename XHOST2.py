...
# ==========================================================
#  FLASK - ANTRAX HOSTING
# ==========================================================
app = Flask('')

DASHBOARD_HTML = """
<!DOCTYPE html><html><head><title>Antrax Hosting</title>
...
<p style="margin-top:30px;color:#444;font-size:0.85em;text-align:center">Antrax Hosting - Powered by AI Seek</p>
</div></body></html>
"""
...
# Public URL for API link button (Railway auto-provides this)
PUBLIC_URL = os.environ.get('RAILWAY_PUBLIC_DOMAIN', '')
if PUBLIC_URL and not PUBLIC_URL.startswith('http'):
    PUBLIC_URL = f"https://{PUBLIC_URL}"
if not PUBLIC_URL:
    PUBLIC_URL = f"http://localhost:{os.environ.get('PORT', 8080)}"
...
SCRIPT_TEMPLATES = {
    'inline_bot': {
        'name': 'InlineKeyboard Bot (Full)',
        'lang': 'py',
        'code': '''# Full InlineKeyboard Telegram Bot - Antrax Hosting
import os
import sys
import time
import traceback
...
def on_callback(c):
    try:
        if c.data == 'menu_status':
            ...
            bot.send_message(c.message.chat.id, "InlineKeyboard bot template by Antrax Hosting")
...
'''
    ...
    'echo_bot': {
        'name': 'Echo Bot (Simple)',
        'lang': 'py',
        'code': '''# Echo Bot - Antrax Hosting
import os
import telebot
...
'''
    ...
    'web_scraper': {
        'name': 'Web Scraper',
        'lang': 'py',
        'code': '''# Web Scraper - Antrax Hosting
import requests
import time
from datetime import datetime
...
'''
    ...
    'data_logger': {
        'name': 'Data Logger',
        'lang': 'py',
        'code': '''# Data Logger - Antrax Hosting
import time
import json
import random
from datetime import datetime
...
'''
    ...
    'cron_worker': {
        'name': 'Cron Worker',
        'lang': 'py',
        'code': '''# Cron Worker - Antrax Hosting
import time
from datetime import datetime
...
def clean_ai_code(raw):
    ...
    # Remove other XML-like reasoning tags
...
def create_main_menu_inline(uid):
    ...
    # API Dashboard link button
    if PUBLIC_URL and PUBLIC_URL.startswith('http'):
        mk.add(types.InlineKeyboardButton('API Dashboard', url=f"{PUBLIC_URL}/dashboard"))
        mk.add(types.InlineKeyboardButton('API Health', url=f"{PUBLIC_URL}/health"))
    if uid in admin_ids:
        mk.add(
...
def cb_buy_tier(call):
    ...
    try:
        ...
        bot.send_invoice(
            chat_id=uid,
            title=f"Antrax Hosting {tier['name']} Tier",
...
6. If it's a Telegram bot, use telebot (pyTelegramBotAPI).
7. For Telegram bots, ALWAYS include InlineKeyboardMarkup with buttons for main commands.
8. Read secrets from os.environ (never hardcode real tokens).
9. Wrap main loop in try/except so it never crashes silently.
10. Add short inline comments.
...
def _logic_send_welcome(msg):
    ...
    text = (f"Welcome to Antrax Hosting, {name}!\n\n"
...
def _logic_stats(msg):
    ...
    text = (f"Antrax Hosting Statistics\n\nUsers: {total_users}\nFiles: {total_files}\n"
...
def _logic_referral(msg):
    ...
    mk = types.InlineKeyboardMarkup()
    share_url = f"https://t.me/share/url?url={link}&text=Try Antrax Hosting bot!"
...
def _logic_api_dashboard(msg):
    """Show API dashboard link."""
    uid = msg.from_user.id
    text = (f"Antrax Hosting API\n\n"
...
def cb_backmain(call):
    uid = call.from_user.id
    text = f"Welcome to Antrax Hosting, {call.from_user.first_name}!\n\nID: {uid}\nTier: {TIERS[get_user_tier(uid)]['name']}"
    try:
        bot.answer_callback_query(call.id)
    ...
    if match:
        ...
        if os.path.exists(lp):
            try:
                ...
                q = (f"Ito ang logs ng script ko. Hanapin ang error at ipaliwanag kung paano ayusin. "
